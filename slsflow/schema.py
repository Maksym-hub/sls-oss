...
Serialization
    ...
    to_glue_string(t: SlsflowType) -> str
    type_from_string(s: str) -> SlsflowType   (Glue-format parser)
...
@dataclass(frozen=True)
class BinaryType(SlsflowType):
    """Variable-length byte array. Glue: binary."""


@dataclass(frozen=True)
class FixedBinaryType(SlsflowType):
...
@dataclass(frozen=True)
class ArrayType(SlsflowType):
    """Array of homogeneous type. Glue: array<inner>."""
    inner: SlsflowType


@dataclass(frozen=True)
class StructType(SlsflowType):
    ...
    """
    fields: Tuple[Tuple[str, SlsflowType], ...]
...
def array(inner: SlsflowType) -> ArrayType:
    if not isinstance(inner, SlsflowType):
        raise TypeError(f"array inner must be a SlsflowType, got {type(inner).__name__}")
    return ArrayType(inner=inner)


def struct(fields: Optional[Mapping[str, SlsflowType]] = None, /, **kwargs: SlsflowType) -> StructType:
    """Build a struct type.
    ...
    if not items:
        raise ValueError("struct must have at least one field")
    pairs: List[Tuple[str, SlsflowType]] = []
    for name, t in items.items():
        if not isinstance(t, SlsflowType):
            raise TypeError(f"struct field {name!r} must be a SlsflowType, got {type(t).__name__}")
        pairs.append((name, t))
    return StructType(fields=tuple(pairs))


def map_(key: SlsflowType, value: SlsflowType) -> MapType:
    """Build a map type. Trailing underscore to avoid shadowing the `map` builtin."""
    if not isinstance(key, SlsflowType):
        raise TypeError(f"map key must be a SlsflowType, got {type(key).__name__}")
    if not isinstance(value, SlsflowType):
        raise TypeError(f"map value must be a SlsflowType, got {type(value).__name__}")
...
class Column:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.type, SlsflowType):
            raise TypeError(
                f"Column.type must be a SlsflowType instance, got {type(self.type).__name__}. "
...
def to_glue_string(t: SlsflowType) -> str:
    ...
    if isinstance(t, TimestampType):
        # Glue itself has only `timestamp`; the tz suffix is a slsflow extension
    ...
    raise TypeError(f"Unknown SlsflowType: {type(t).__name__}")
...
_SIMPLE_TYPE_PARSERS: Dict[str, SlsflowType] = {
    "tinyint": TinyIntType(),
    "byte": TinyIntType(),
...
def type_from_string(s: str) -> SlsflowType:
    """Parse a Glue / Hive-style type string into a SlsflowType.
    ...
    if text.startswith("struct<") and text.endswith(">"):
        ...
        fields: List[Tuple[str, SlsflowType]] = []
        for part in parts:
...
RawColumn = Union[
    Column,
    Tuple[str, Union[str, SlsflowType]],
    Tuple[str, Union[str, SlsflowType], str],
    Mapping[str, Any],
]
...
def normalize_schema(raw: Optional[Sequence[Any]]) -> Schema:
    ...
    Accepted input shapes per entry:
        ...
        - Tuple (name, type)              — type is str or SlsflowType
        - Tuple (name, type, description) — type is str or SlsflowType
...
def _normalize_entry(entry: Any) -> Column:
    ...
    if isinstance(entry, (list, tuple)):
        ...
        name = entry[0]
        type_arg = entry[1]
        type_obj = type_arg if isinstance(type_arg, SlsflowType) else type_from_string(type_arg)
...
def dict_schema_richness(schema: Sequence[Mapping[str, Any]]) -> int:
    ...
    Used by the backend conflict-resolution path (`routes/assets.py`),
    which works with already-serialized schema dicts read from
...
def _slsflow_type_to_jsonschema(t: SlsflowType, nullable: bool) -> Dict[str, Any]:
    """Convert a SlsflowType to a JSON Schema fragment.
    ...
    if isinstance(t, ArrayType):
        ...
        return _ann({"type": "array", "items": _slsflow_type_to_jsonschema(t.inner, False)})
    if isinstance(t, MapType):
        # JSON objects always have string keys; surface non-string key types
        # via a propertyNames format hint without rejecting them.
        out: Dict[str, Any] = {
            "type": "object",
            "additionalProperties": _slsflow_type_to_jsonschema(t.value, False),
        }
        if not isinstance(t.key, StringType):
            out["propertyNames"] = _slsflow_type_to_jsonschema(t.key, False)
        return _ann(out)
    if isinstance(t, StructType):
        props: Dict[str, Any] = {}
        for name, ft in t.fields:
            props[name] = _slsflow_type_to_jsonschema(ft, False)
        return _ann({"type": "object", "properties": props,
                     "required": [n for n, _ in t.fields]})

    raise TypeError(f"Unknown SlsflowType in JSON Schema export: {type(t).__name__}")
