...
# Sentinel for required parameter
class _Required:
...
class DAG:
    ...
    Alerts configuration:
        alerts={"slack": "#channel"}                      # Slack only
        alerts={"pagerduty": "critical"}                  # PagerDuty only  
        alerts={"slack": "#channel", "pagerduty": "error"} # Both
        alerts={"slack": "#ch", "slack_mentions": ["YOUR_SLACK_USER_ID"]} # With mentions
        alerts=None                                        # Explicitly disabled
    
    PagerDuty severity levels: critical, error, warning, info
    """
    dag_id: str
    description: str = ""
    
    # Alerts - REQUIRED parameter
    # Must be dict with "slack" and/or "pagerduty", or None to disable
    # IDE: see AlertsDict for autocomplete on keys
    alerts: Union["AlertsDict", None, _Required] = REQUIRED
    ...
    schedule: Any = None
    schedule_interval: str = None  # Alias for schedule (deprecated in Airflow 2.4+)
    timetable: Any = None  # Custom timetable
    trigger_assets: List[Any] = None  # Alternative to schedule for asset triggers
    ...
    default_timeout: int = None  # Default timeout for all tasks (seconds)
    ...
    # Rendering
    template_searchpath: List[str] = None
    ...
    _eventbridge_schedule: str = field(default=None, repr=False)
    _asset_schedule: Any = field(default=None, repr=False)  # Normalized asset schedule
    
    def __post_init__(self):
        # Validate alerts parameter - REQUIRED
        if isinstance(self.alerts, _Required):
            raise ValueError(
                f"DAG '{self.dag_id}': 'alerts' parameter is required.\n"
                f"\n"
                f"Examples:\n"
                f"  alerts={{'slack': '#channel'}}                       # Slack only\n"
                f"  alerts={{'slack': '#ch', 'slack_mentions': ['U...']}} # With mentions\n"
                f"  alerts={{'pagerduty': 'critical'}}                   # PagerDuty only\n"
                f"  alerts={{'slack': '#channel', 'pagerduty': 'error'}}  # Both\n"
                f"  alerts={{'slack': '#ch', 'slack_mentions': ['YOUR_SLACK_USER_ID']}}  # With mentions\n"
                f"  alerts=None                                          # Explicitly disabled"
            )
    ...
    def test(self, execution_date: datetime = None):
