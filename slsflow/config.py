"""
Centralized configuration for slsflow.

Configuration is loaded from (in priority order):
1. CLI arguments  (--stage, --profile, --namespace)
2. Environment variables  (SLSFLOW_NAMESPACE, SLSFLOW_STAGE, SLSFLOW_REGION)
...
def _exec_config_module(config_path: Path):
    ...
    """
    spec = importlib.util.spec_from_file_location("_slsflow_config", config_path)
...
def _find_project_config() -> Optional[Path]:
    ...
    for directory in [current, *current.parents]:
        ...
        if not config_path.exists():
            continue
        # Make sure it's a slsflow config (has ENVIRONMENTS key)
        try:
...
class _RolesDict(dict):
    ...
    def __getitem__(self, key: str) -> str:
        env_var = f"SLSFLOW_ROLE_{key.upper()}"
...
class SlsflowConfig:
    """
    Central configuration for slsflow.
    ...
    def namespace(self) -> str:
        env_value = os.environ.get("SLSFLOW_NAMESPACE")
        if env_value:
            return env_value
        return self._env_config().get("namespace", "slsflow")

    @property
    def stage(self) -> str:
        env_value = os.environ.get("SLSFLOW_STAGE")
        if env_value:
            return env_value
    ...
    @property
    def region(self) -> str:
        env_value = os.environ.get("SLSFLOW_REGION") or os.environ.get("AWS_REGION")
        if env_value:
            return env_value
    ...
    def profile(self) -> Optional[str]:
        env_value = os.environ.get("SLSFLOW_PROFILE") or os.environ.get("AWS_PROFILE")
        if env_value:
            return env_value
    ...
    def for_stage(self, stage: str) -> "SlsflowConfig":
        """Get config view for a specific stage (used by slsflow-deploy --stage)."""
        view = _StageConfig(self, stage)
        return view

    def get(self, key: str, default: Any = None) -> Any:
        env_var = f"SLSFLOW_{key.upper()}"
        env_value = os.environ.get(env_var)
        if env_value:
            return env_value
    ...
    def reset(self) -> None:
        SlsflowConfig._loaded = False
        self._environments = {}
        self._default_stage = "dev"


class _StageConfig:
    """Config view for a specific stage — used by slsflow-deploy --stage."""

    def __init__(self, parent: SlsflowConfig, stage: str):
        self._parent = parent
        self._stage = stage
    ...
    @property
    def namespace(self) -> str:
        return os.environ.get("SLSFLOW_NAMESPACE") or self._env_config().get("namespace", "slsflow")
    ...
    @property
    def region(self) -> str:
        return (os.environ.get("SLSFLOW_REGION") or os.environ.get("AWS_REGION")
                or self._env_config().get("region", "us-east-1"))

    @property
    def profile(self) -> Optional[str]:
        return (os.environ.get("SLSFLOW_PROFILE") or os.environ.get("AWS_PROFILE")
