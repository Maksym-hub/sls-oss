...
CLI:
    slsflow local my_dag.py --dry-run
    slsflow local my_dag.py --validate
    slsflow local my_dag.py --mock
    slsflow local my_dag.py --localstack
"""
...
def _evaluate_trigger_rule(task: Task, previous_results: List[TaskResult]) -> tuple:
    ...
    upstream_ids = {t.task_id for t in task.dependencies}
