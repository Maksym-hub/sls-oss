...
Usage:
    from slsflow.validation import validate_all
...
def extract_dag_info(file_path: str) -> List[DAGInfo]:
    ...
    try:
        ...
        try:
            ...
            for mod in ['pipelines', 'pipelines.config']:
...
def detect_asset_cycles(graph: Dict) -> List[Dict]:
    """
    Detect cycles in asset trigger graph.
    
    A cycle exists when:
    DAG_A produces asset_x → triggers DAG_B → produces asset_y → triggers DAG_A
    
    Args:
        graph: Asset graph from build_asset_graph()
        
    Returns:
        List of cycle info dicts with 'path' and 'description'
    """
    cycles = []
    
    # Get all DAGs that are asset-triggered
    triggered_dags = list(graph['dag_consumes'].keys())
    
    for start_dag in triggered_dags:
        # BFS to find cycles starting from this DAG
        visited = set()
        queue = [(start_dag, [start_dag])]
        
        while queue:
            current_dag, path = queue.pop(0)
            
            if current_dag in visited and current_dag != start_dag:
                continue
            
            # Get assets this DAG produces
            produced = graph['dag_produces'].get(current_dag, [])
            
            for asset in produced:
                # Get DAGs triggered by this asset
                triggered = graph['consumers'].get(asset, [])
                
                for next_dag in triggered:
                    new_path = path + [f"→ {asset} →", next_dag]
                    
                    # Cycle found!
                    if next_dag == start_dag and len(path) > 1:
                        cycle_desc = ' '.join(new_path)
                        cycle_info = {
                            'start_dag': start_dag,
                            'path': new_path,
                            'description': cycle_desc
                        }
                        # Avoid duplicate cycles (A→B→A same as B→A→B)
                        if not any(c['start_dag'] == start_dag for c in cycles):
                            cycles.append(cycle_info)
                    
                    elif next_dag not in visited:
                        visited.add(next_dag)
                        queue.append((next_dag, new_path))
...
def main():
    """
    CLI entry point for slsflow-validate command.

    Usage:
        slsflow-validate                    # Validate dag.py in current directory
        slsflow-validate -v                 # Verbose output
        slsflow-validate --all              # Validate all pipelines found
        slsflow-validate --all -v           # All pipelines, verbose
        slsflow-validate --json             # Output as JSON
    """
    import argparse
    import json

    parser = argparse.ArgumentParser(
        prog='slsflow-validate',
        description='Validate slsflow pipeline(s)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  slsflow-validate              # Validate dag.py in current directory
  slsflow-validate -v           # Verbose output with details
  slsflow-validate --all        # Find and validate all pipelines
  slsflow-validate --all -v     # All pipelines, verbose
  slsflow-validate --json       # Output as JSON"""
    )
    parser.add_argument(
