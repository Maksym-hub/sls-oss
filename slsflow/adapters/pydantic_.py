"""pydantic.BaseModel  →  List[slsflow.Column].
...
slsflow Asset. This adapter eliminates the redeclaration.
...
Type mapping (pydantic / Python typing  →  slsflow):
...
def _require_pydantic():
    ...
    try:
        ...
        raise ImportError(
            "pydantic is required for slsflow.adapters.pydantic_. "
            "Install with:  pip install 'slsflow[pydantic]'"
        ) from e
...
def pydantic_to_columns(model_cls: type) -> Schema:
    ...
    if fields is None:
        raise TypeError(
            ...
            f"slsflow requires pydantic v2 (>=2.0)."
        )

    out: List[Column] = []
    for name, field_info in fields.items():
        annotation = field_info.annotation
        slsflow_type, is_optional = _annotation_to_slsflow(annotation)
...
def _annotation_to_slsflow(ann: Any) -> Tuple[SlsflowType, bool]:
    """Resolve a Python type annotation to (slsflow_type, is_optional).
    ...
    if origin is Union:
        ...
        if len(non_none) == 1:
            inner_type, _ = _annotation_to_slsflow(non_none[0])
            return inner_type, is_optional
        # Multiple non-None members (Union[int, str, ...]) — slsflow has no
    ...
    if origin in (list, _typing.List):
        if args:
            inner_type, _ = _annotation_to_slsflow(args[0])
            return array(inner_type), False
        return array(string()), False

    if origin in (dict, _typing.Dict):
        if len(args) == 2:
            k_type, _ = _annotation_to_slsflow(args[0])
            v_type, _ = _annotation_to_slsflow(args[1])
