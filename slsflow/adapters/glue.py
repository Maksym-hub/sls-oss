"""AWS Glue Catalog  →  List[slsflow.Column].
...
def glue_table_to_columns(
    ...
    *,
    ...
    for col in (sd.get("Columns") or []):
        out.append(_glue_column_to_slsflow(col, partition_key=False))
    # Partition columns
    for col in (table_obj.get("PartitionKeys") or []):
        out.append(_glue_column_to_slsflow(col, partition_key=True))

    return out


def _glue_column_to_slsflow(glue_col: dict, *, partition_key: bool) -> Column:
    ...
    type slsflow knows how to emit it can also read back from Glue without
...
#   year/month          → monthly
#
# This inference is **advisory only**. At deploy time, `slsflow-deploy`
...
def fetch_glue_partition_keys(
    ...
    *,
    ...
    Used by `slsflow-deploy` to infer granularity without parsing every
