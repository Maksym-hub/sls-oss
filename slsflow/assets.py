...
from typing import List, Literal, Optional, Dict, Any, Union, TYPE_CHECKING
from dataclasses import dataclass, field

from .schema import (
    Column, Schema, column_to_dict, normalize_schema,
    _slsflow_type_to_jsonschema,
)
...
class Asset:
    ...
    Args:
        ...
        description: Human-readable description (slsflow extension)
        tags: Optional list of tags for filtering/categorization (slsflow extension)
        freshness_hours: Optional freshness threshold in hours (slsflow extension)
        owner: Optional owner team or person (slsflow extension)
        schema: Optional column schema. Accepted forms (mixed in one list is allowed):
                ...
                See `slsflow.schema` for the full type system (types.bigint(), types.decimal(10, 2), etc.)
                and `slsflow.Column` for the column class with constraints
        ...
        federated source (Lambda-backed Hive/MySQL/Snowflake). SLSFlow
    ...
    def __init__(
        ...
        *,
        ...
        # slsflow extensions (not in Airflow)
        description: str = "",
    ...
    # slsflow.adapters; they exist so users get the natural call site
    ...
    def from_parquet(
        ...
        **kwargs: Any,
        ...
        Requires the `pyarrow` extra (`pip install 'slsflow[pyarrow]'`);
    ...
    def from_pydantic(
        ...
        **kwargs: Any,
        ...
        are all surfaced. See `slsflow.adapters.pydantic_` for the full
        type-mapping table.
        ...
        **kwargs: Any,
        ...
        Args:
            iceberg_table: a `pyiceberg.table.Table` instance. The
                pyiceberg package is NOT a slsflow dependency — the
        ...
        `from_pyarrow`). Install with `pip install 'slsflow[pyarrow]'`.
    ...
    def to_ddl(self, dialect: str = 'glue') -> str:
        ...
        the renderers move to `slsflow/renderers/` as a plugin pattern;
    ...
    def to_jsonschema(self) -> Dict[str, Any]:
        ...
        required: List[str] = []
        for col in self.schema:
            properties[col.name] = _slsflow_type_to_jsonschema(col.type, col.nullable)
            if col.description:
...
class AssetAll:
    ...
    def __and__(self, other: Union[Asset, 'AssetAll']) -> 'AssetAll':
        """Chain AND: (a & b) & c"""
        if isinstance(other, AssetAll):
            return AssetAll(assets=self.assets + other.assets)
        elif isinstance(other, Asset):
            return AssetAll(assets=self.assets + [other])
    ...
    def __or__(self, other) -> 'AssetAny':
        ...
        if isinstance(other, (Asset, AssetAny)):
            return AssetAny(assets=[self, other])
        raise TypeError(f"Cannot combine AssetAll with {type(other)}")
...
class AssetAny:
    ...
    """
    assets: List[Union[Asset, AssetAll]] = field(default_factory=list)
    
    def __or__(self, other: Union[Asset, 'AssetAny', AssetAll]) -> 'AssetAny':
        """Chain OR: (a | b) | c"""
        if isinstance(other, AssetAny):
            return AssetAny(assets=self.assets + other.assets)
        elif isinstance(other, (Asset, AssetAll)):
            return AssetAny(assets=self.assets + [other])
...
def normalize_asset_schedule(schedule: AssetSchedule) -> Union[AssetAll, AssetAny, None]:
    """
    ...
    if isinstance(schedule, AssetAlias):
        return AssetAny(assets=schedule.assets)
    ...
    if isinstance(schedule, list):
        ...
        if len(schedule) == 1:
            ...
            if isinstance(item, (AssetAll, AssetAny)):
                ...
                return AssetAny(assets=item.assets)
            elif isinstance(item, Asset):
                return AssetAll(assets=[item])
...
# renderers move to a `slsflow/renderers/` package and `to_ddl` becomes a
