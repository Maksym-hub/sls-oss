#!/usr/bin/env python3
"""
Register slsflow pipeline manually.
...
Usage:
    ...
    slsflow-register --name my-pipeline --role-arn arn:aws:iam::123:role/deploy-role
"""
...
def get_sfn_client(region: str, profile: str = None, role_arn: str = None):
    """
    ...
    try:
        if role_arn:
            ...
            response = sts.assume_role(
                RoleArn=role_arn,
                RoleSessionName='slsflow-register'
            )
...
def register_pipeline(sfn_arn: str, region: str = None, profile: str = None, role_arn: str = None) -> dict:
    """
    ...
    try:
        ...
        response = sfn.start_execution(
            ...
            input=json.dumps({
                'register_only': True,
                'triggered_by': 'slsflow-register-cli'
            })
        )
...
def find_sfn_arn(name: str, region: str, namespace: str = None, profile: str = None, role_arn: str = None) -> str:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Register slsflow pipeline (writes to DynamoDB without running tasks)',
...
Examples:
    ...
    slsflow-register --name my-pipeline --profile dev --role-arn arn:aws:iam::456:role/prod-deploy
