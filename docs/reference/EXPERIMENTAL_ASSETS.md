...
> **⚠️ Assets are DISABLED by default as of the v1 release.** `Asset(...)`
> construction raises unless `_ASSETS_ENABLED = True` in `polyris/assets.py`
> (or `POLYRIS_ENABLE_ASSETS=1` is exported, which the test suite does). **To
> re-enable assets in a later release, flip that one flag** — every downstream
> asset path is data-driven, so nothing else in the runtime needs to change.
> This is step 1 of the checklist below.
...
1. **Enable the feature** — `polyris/assets.py`
   Set `_ASSETS_ENABLED = True` (and remove the `POLYRIS_ENABLE_ASSETS` env
   override + the root `conftest.py` opt-in if no longer wanted). This is the one
   switch that gates all `Asset` construction — nothing downstream changes. Also
   restore the asset symbols (`Asset`, `AssetAll`, `AssetAny`, `AssetAlias`) to
   `__all__` in `polyris/__init__.py`.

2. **Runtime warning** — `polyris/assets.py`
...
3. **Doc banners** — `README.md`, `docs/features/ASSETS.md`,
